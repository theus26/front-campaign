...
import { IProvider, IProviderGeneric } from "@/@core/services/interfaces/campaign/IProviderService";
...
const defaultItem = ref<IProviderGeneric>({
  id: -1,
  providerId: "",
  credential: "",
  name: "",
...
const provider: IProviderGeneric[] = [];

const img = ref("")
const pro = ref<IProviderGeneric[]>([]);
...
const reconectarProvider = (item: IProviderGeneric) => {
...
const editarItem = (item: IProviderGeneric) => {
  editedIndex.value = pro.value.indexOf(item);
  editedItem.value = { ...item };
  editarProviderDialog.value = true;
};

const providers = computed((): IProviderGeneric[] => {
...
const loadProviders = async () => {
  try {
    ...
    console.log(data);
    provider.push(...data); // adiciona todos os itens se for array
...
const desconectarConexao = async (provider: IProviderGeneric) => {
  ...
  editedItem.value = { ...provider };
  desconectarDialog.value = true;
  console.log(editedItem.value.instanceName)
};
  ...
  <VCard>
        ...
        <VWindowItem>
          <VForm ref="refAddressForm" @submit.prevent="validateAddressForm">
            <VRow>
              ...
              <VCol cols="12" v-if="!providers.length">
              ...
              <VCol v-if="providers.length" cols="12">
                <VDivider />
      ...
      <VCard title="Editar Conexão">
        <VCardText>
          ...
          <div class="self-align-end d-flex gap-2 justify-end mt-3 mb-1">
            ...
            <VBtn type="submit" @click="editarProviderDialog = false">
              Salvar
            </VBtn>
          </div>
        </VCardText>
    ...
    <VDialog :width="$vuetify.display.smAndDown ? 'auto' : 600" v-model="addProviderDialog" max-width="600px">
      ...
      <VCard title="Adicionar nova Conexão">
        <VCardText>
          <VCol cols="12">
            <AppTextField label="Informe nome da conexão *" v-model="nameProvider" placeholder="Nome"
